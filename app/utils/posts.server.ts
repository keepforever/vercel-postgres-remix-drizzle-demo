...
export const updatePost = async (payload: UpdatePostPayload) => {
  ...
  return await db.update(posts).set({ name }).where(eq(posts.id, id)).returning({ id: posts.id, name: posts.name })
}
...
export const getPostWithAuthor = async (id: number) => {
  return await db.select().from(posts).leftJoin(user, eq(posts.authorId, user.id)).where(eq(posts.id, id))
}
